...
from .models import CaseReport,CaseReportTwo
...
class CaseReportAdmin(object):
    ...
    list_filter = ['test_project','test_module']   #定义筛选的字段
...
xadmin.site.register(CaseReport, CaseReportAdmin) #在xadmin中注册CaseReport


class CaseReportTwoAdmin(object):
    ziduan = ['test_project','test_module','is_repeat','add_time']
    ...
    def queryset(self):   #重载queryset方法，用来做到不同的admin取出的数据不同
        qs = super(CaseReportTwoAdmin,self).queryset()   #调用父类
        qs = qs.filter(is_repeat=False)  #只过滤显示不重复的数据
        return qs   #返回qs

xadmin.site.register(CaseReportTwo, CaseReportTwoAdmin) #在xadmin中注册CaseReport
